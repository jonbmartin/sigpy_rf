...
class TestOptcont(unittest.TestCase):
    ...
    def test_rf_autodiff(self):
        ...
        if excute == 'y':
            ...
            if excute == 'y':
                ...
                pbc = b1[np.floor(b1.size / 2).astype(int)]  # b1 (Gauss)
                pbw = b1[-1] / 1.5  # b1 (Gauss)
                rfp_bs, rfp_ss, _ = rf.dz_bssel_rf(dt=dt, tb=2, ndes=256, ptype='st',
                                                   flip=np.pi / 4,
                                                   pbw=pbw,
                                                   pbc=[pbc], d1e=0.01, d2e=0.01,
                                                   rampfilt=True, bs_offset=5000)
                ...
                for ii in range(nb1):
                    Mx_ini[ii], My_ini[ii], Mz_ini[ii] = rf.sim.arb_phase_b1sel_loop(rf_op, b1[ii],
                                                                                     0,
                                                                                     0, 1.0, nt)
                print('Finish setting up initial pulse and profile. Pulse duration: {:f}ms. '
                      'Time: {:f}'.format(np.size(full_pulse) * dt * 1000, (time.time() - t0)))
            ...
            while proceed == 'y':
                ...
                # generate magnetization profile with acquired pulse
                for ii in range(nb1):
                    Mxi[ii], Myi[ii], Mzi[ii] = rf.sim.arb_phase_b1sel_loop(rf_op_test_5, b1[ii], 0,
                                                                            0,
                                                                            1.0, nt)
    ...
    def test_rf_autodiff_mxy(self):
        t0 = time.time()
        print('Test start.')

        excute = input("Start with new pulse? (y/n):\n")
        if excute == 'y':
            name = input("New pulse name (file name only, not including .mat):\n")
            # generate initial pulse
            dt = 1e-6
            b1 = np.arange(0, 1, 0.02)  # gauss, b1 range to sim over
            nb1 = np.size(b1)
            ...
            for ii in range(nb1):
                Mxi[ii], Myi[ii], Mzi[ii] = rf.sim.arb_phase_b1sel_loop(rf_op, b1[ii], 0,
                                                                        0, 1.0, nt)
            print('Finish setting up initial pulse and profile. Pulse duration: {:f}ms. '
            ...
            # set up target
            # Mxyd = np.ones(nb1)
            Mxyd = np.ones(nb1) * 0.7
            # w = np.ones(nb1)
            w = np.append(np.zeros(10), np.ones(nb1 - 10))
            # w = np.append(w, np.zeros(10))  # weight
            niters = 0
            step_size = 0.00010
            loss = 0
            ...
            b1 = np.squeeze(dict['b1'])  # gauss, b1 range to sim over
            nb1 = np.size(b1)
            run_time = np.squeeze(dict['run_time'])

            pulse_ini = np.squeeze(dict['pulse_ini'])
            full_pulse = np.squeeze(dict['pulse_fin'])
            Mxi = np.squeeze(dict['Mx_fin'])
            Myi = np.squeeze(dict['My_fin'])
            Mzi = np.squeeze(dict['Mz_fin'])
        ...
        while proceed == 'y':
            # optimize test pulse
            full_pulse, new_loss = optcont.rf_autodiff_mx_my(
                full_pulse, b1, Mxyd, w, new_iters, step_size, mx0=0, my0=0, mz0=1.0)

            loss = np.append(loss, new_loss)
            niters += new_iters
            print('Finish optimization. Current total number of iters: {:d} Time: {:f}s.'.format(
                niters, time.time() - t0))
            ...
            nt = np.size(rfp_abs)
            rf_op = np.append(rfp_abs, rfp_angle)
            Mxy_ini = np.sqrt(Mxi ** 2 + Myi ** 2)
            for ii in range(nb1):
                Mxi[ii], Myi[ii], Mzi[ii] = rf.sim.arb_phase_b1sel_loop(rf_op, b1[ii], 0, 0,
                                                                        1.0, nt)

            # graphs (temp)
            pyplot.figure()
            pyplot.plot(b1, np.sqrt(Mxi ** 2 + Myi ** 2), '-r', label='Mxy')
            pyplot.plot(b1, Mxy_ini, '-g', label='Mxy initial')
            # pyplot.plot(b1, np.sqrt(Mxd ** 2 + Myd ** 2), '-b', label='Mxy desired')
            pyplot.plot(b1, Mzi, '-c', label='Mz')
            pyplot.legend()
            pyplot.show()
            ...
            pyplot.figure()
            pyplot.plot(abs(pulse_ini).T, '-r', label='rf pulse initial')
            pyplot.plot(abs(full_pulse).T, '-g', label='rf pulse final')
            pyplot.legend()
            pyplot.show()
        ...
        # save the pulse
        print('Saving Results')
        dict_out = {'Mx_fin': Mxi, 'My_fin': Myi, 'Mz_fin': Mzi, 'Mxy_dsr': Mxyd,
                    'pulse_ini': pulse_ini, 'pulse_fin': full_pulse,
                    'niters': niters, 'step_size': step_size,
